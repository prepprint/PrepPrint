...
export function FileUpload() {
  ...
  useEffect(() => {
    ...
    let items = [], particles = [], splats = [], combos = [], slashTrail = [];   
    const targetEmojis = ['⏰', '🧮', '📚', '📝', '✏️'];
    let scoreCounter = 0, sliceTimestamps = [];
  ...
  const generateThumbnails = async (fileObj, id) => {
    ...
    } catch (err) { console.error("Error drawing client thumbnails:", err); }
  };
  ...
  const processSingleFile = async (fileId, currentFile) => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/v1/process-pdf`, { method: 'POST', body: formData });
  ...
  const processMergedFiles = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/v1/merge-pdfs`, { method: 'POST', body: formData });
      clearInterval(progressInterval);
  ...
  return (
    <div className="w-full max-w-5xl mx-auto mt-8 px-4">
      ...
      <div className="col-span-1 sm:col-span-2 lg:col-span-4 mt-2 p-4 bg-slate-50 dark:bg-slate-900/40 rounded-xl border border-gray-200 dark:border-slate-800">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="min-w-0">
            <h4 className="text-xs font-black uppercase tracking-wider text-gray-500 dark:text-slate-400 flex items-center gap-1.5"><Zap className="w-3.5 h-3.5 text-blue-500" />Live Print Sheet Preview</h4>
            <p className="text-[11px] text-gray-400 mt-0.5">Showing arrangement for A4 Page (Current Selection: <span className="font-bold text-blue-500">{nUp}-Up</span>)</p>
          </div>
          <div className="flex-shrink-0 bg-white dark:bg-slate-950 border border-gray-300 dark:border-slate-700 shadow-md rounded transition-all duration-200 p-2 flex items-center justify-center relative overflow-hidden"
            style={{ width: orientation === 'landscape' ? '110px' : '78px', height: orientation === 'landscape' ? '78px' : '110px', paddingLeft: gutterMargin === 'left' || gutterMargin === 'alternating' ? '12px' : '6px' }}>
            ...
            <div className="w-full h-full grid gap-1 transition-all duration-200"
              style={{ gridTemplateColumns: `repeat(${nUp === 1 ? 1 : nUp === 2 ? (orientation === 'landscape' ? 2 : 1) : nUp === 3 ? 1 : nUp === 4 ? 2 : nUp === 6 ? (orientation === 'landscape' ? 3 : 2) : nUp === 8 ? (orientation === 'landscape' ? 4 : 2) : nUp === 9 ? 3 : nUp === 12 ? (orientation === 'landscape' ? 4 : 3) : 4 }, minmax(0, 1fr))`,
      ...
      {!pdfJsLoaded ? (
        <div className="h-40 border-2 border-dashed border-gray-200 rounded-xl flex items-center justify-center p-6 bg-gray-50 dark:bg-gray-800/10"><Loader2 className="w-6 h-6 animate-spin text-blue-500 mr-2" /><span className="text-sm font-medium text-gray-500">Initializing document workspace...</span></div>
      ) : files.length < 25 && (
        <div {...getRootProps()} className={`h-40 border-2 border-dashed rounded-xl flex flex-col items-center justify-center p-6 cursor-pointer mb-6 transition-all ${isGlobalProcessing ? "opacity-50 pointer-events-none" : "border-gray-300 dark:border-gray-700 hover:border-blue-500 hover:bg-gray-50 dark:hover:bg-gray-800/50"}`}>
          <input {...getInputProps()} /><UploadCloud className="w-10 h-10 mb-3 text-gray-400" /><p className="font-medium text-gray-700 dark:text-gray-200 text-center">Drag up to 25 PDFs here</p>
        </div>
      )}
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-4 shadow-sm transition-colors">
          {files.length > 1 && (
            <div className="flex items-center mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-100 dark:border-blue-800/50">
              <input type="checkbox" id="mergeToggle" checked={isMerging} onChange={(e) => setIsMerging(e.target.checked)} disabled={isGlobalProcessing || allCompleted} className="w-4 h-4 text-blue-600 rounded cursor-pointer" />
              <label htmlFor="mergeToggle" className="ml-3 text-sm font-medium text-gray-700 dark:text-gray-300 cursor-pointer flex items-center"><Layers className="w-4 h-4 mr-2" />Merge all into a single PDF</label>
            </div>
          )}
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex flex-col items-center justify-center p-4 select-none">
          <div className="w-full max-w-xl bg-slate-900/40 border border-slate-800 rounded-2xl shadow-2xl p-6 flex flex-col items-center backdrop-blur-sm">
            <div className="w-full flex justify-between items-center mb-4 pb-3 border-b border-slate-800">
              <div className="flex items-center space-x-2"><Zap className="w-4 h-4 text-blue-500 animate-bounce" /><h3 className="font-extrabold text-slate-200 text-sm tracking-wide uppercase">Exam Rage: Performance Core</h3></div>
              <div className="bg-blue-500/10 text-blue-400 text-xs font-black px-3 py-1 rounded-lg tracking-wider">SCORE: {gameScore}</div>
            </div>
            <div className="relative border border-slate-800/80 bg-slate-950 rounded-xl overflow-hidden cursor-crosshair shadow-inner w-full"><canvas ref={canvasRef} className="block w-full" /></div>
            <div className="w-full mt-5 space-y-2">
              <div className="flex justify-between text-[11px] text-slate-400 font-bold px-1 tracking-wide"><span>Inverting Document Color Space...</span></div>
              <div className="w-full bg-slate-950 border border-slate-800 rounded-full h-2 p-0.5 overflow-hidden"><div className="bg-gradient-to-r from-blue-500 to-indigo-600 h-1 rounded-full animate-pulse" style={{ width: '75%' }}></div></div>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="w-full max-w-5xl h-[85vh] bg-white dark:bg-gray-950 rounded-2xl flex flex-col shadow-2xl border border-gray-100 dark:border-gray-800 overflow-hidden animate-in fade-in zoom-in duration-150">
            <div className="p-4 border-b border-gray-200 dark:border-gray-800 flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-gray-50 dark:bg-gray-900/50">
              <div className="min-w-0">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Keeping <span className="font-semibold text-blue-600">{pageMaps[activeModalFile.id].filter(p => p.keep).length}</span> pages · Deleting <span className="font-semibold text-red-500">{pageMaps[activeModalFile.id].filter(p => !p.keep).length}</span> pages</p>
              </div>
              <div className="flex items-center space-x-2 bg-white dark:bg-gray-900 p-1.5 rounded-lg border border-gray-200 dark:border-gray-800 self-start md:self-auto">
                <div className="flex items-center space-x-1 text-xs text-gray-500 font-medium pl-1"><Scissors className="w-3.5 h-3.5 text-blue-500" /><span>Slice Chapter:</span></div>
                <input type="number" placeholder="From" value={splitStart} onChange={(e) => setSplitStart(e.target.value)} className="w-14 px-2 py-1 text-xs text-center border border-gray-300 dark:border-gray-700 bg-transparent rounded text-gray-900 dark:text-white outline-none focus:border-blue-500" />
                <input type="number" placeholder="To" value={splitEnd} onChange={(e) => setSplitEnd(e.target.value)} className="w-14 px-2 py-1 text-xs text-center border border-gray-300 dark:border-gray-700 bg-transparent rounded text-gray-900 dark:text-white outline-none focus:border-blue-500" />
            ...
            <div className="flex-1 overflow-y-auto p-6 bg-gray-100/50 dark:bg-gray-900/20">
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
                {pageMaps[activeModalFile.id].map((page, pIdx) => (
                  <div key={pIdx} onClick={() => togglePageSelection(activeModalFile.id, pIdx)} className={`group relative border rounded-xl overflow-hidden cursor-pointer bg-white dark:bg-gray-900 select-none shadow-sm transition-all duration-200 hover:scale-102 hover:shadow-md ${page.keep ? 'border-gray-200 dark:border-gray-800 ring-2 ring-transparent hover:ring-blue-500' : 'border-red-300 dark:border-red-900 ring-2 ring-red-500 opacity-60'}`}>
                    <div className="aspect-[3/4] flex items-center justify-center p-2 bg-gray-50 dark:bg-gray-900/50"><img src={page.thumbnail} alt={`Page ${page.displayNum}`} className="max-h-full max-w-full object-contain pointer-events-none" /></div>
                    <div className="p-2 border-t border-gray-100 dark:border-gray-800 flex justify-between items-center bg-gray-50 dark:bg-gray-900">
                      ...
                      {page.keep ? (<div className="w-4 h-4 rounded-full border border-gray-300 dark:border-gray-600 flex items-center justify-center group-hover:border-blue-500"><div className="w-2 h-2 rounded-full bg-transparent group-hover:bg-blue-500"></div></div>) : (<div className="w-4 h-4 rounded-full bg-red-500 flex items-center justify-center"><X className="w-2.5 h-2.5 text-white stroke-[4]" /></div>)}
                    </div>
                    {!page.keep && (<div className="absolute inset-0 bg-red-500/10 backdrop-blur-[0.5px] flex flex-col items-center justify-center pointer-events-none"><div className="bg-red-600 text-white font-black text-xs px-2 py-1 rounded shadow flex items-center space-x-1"><Trash2 className="w-3 h-3" /><span>DELETING</span></div></div>)}
                  </div>
                ))}
              </div>
            </div>
